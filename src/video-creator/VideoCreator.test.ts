...
test("test me", async () => {
  ...
  const videoId = videoCreator.addToQueue(
    [
      {
        text: "test",
        searchTerms: ["test"],
  ...
  let videos = videoCreator.listAllVideos();
  ...
  fs.writeFileSync(videoCreator.getVideoPath(videoId), "mock video content");
  videos = videoCreator.listAllVideos();
  expect(videos.find((v) => v.id === videoId)?.status).toBe("processing");
  ...
  const status = videoCreator.status(videoId);
